...
const IcStar     = ({ filled }) => (<svg width="28" height="28" viewBox="0 0 24 24" fill={filled ? "#f59e0b" : "none"} stroke="#f59e0b" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>);
const IcStarSm   = ({ filled }) => (<svg width="16" height="16" viewBox="0 0 24 24" fill={filled ? "#f59e0b" : "none"} stroke="#f59e0b" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>);
...
export default function MenteeDashboard() {
  ...
  const fetchReviewStatus = async (bookingIds) => {
    ...
    setReviewStatus(prev => ({ ...prev, ...results }));
  };
  ...
  const fetchMySessions = async () => {
    try {
      ...
      const completedWithPrice = bData.map(s => ({
        ...s,
        invoice_amount: s.invoice_amount || s.price || 75000,
      }));
      setCompletedSessions(completedWithPrice);
      // Cek status review untuk semua yang sudah paid
  ...
  const refreshAll = async () => {
    await Promise.all([fetchMyBookings(), fetchMySessions()]);
  };
  ...
  const handlePayInvoice = async (bookingId) => {
    ...
    try {
      ...
      window.snap.pay(token, {
        ...
        onPending: () => { showFlash('success', '⏳ Selesaikan pembayaran kamu.'); setPayingId(null); },
        onError:   () => { showFlash('error', '❌ Pembayaran gagal.'); setPayingId(null); },
  ...
  const S = {
    ...
    payBtnD:  { marginTop:'10px', display:'inline-flex', alignItems:'center', gap:'8px', border:'none', padding:'10px 22px', borderRadius:'10px', fontSize:'13px', fontWeight:'700', cursor:'not-allowed', color:'#fff', background:'#aaa' },
    reviewBtn:{ marginTop:'10px', display:'inline-flex', alignItems:'center', gap:'8px', border:'none', padding:'9px 18px', borderRadius:'10px', fontSize:'13px', fontWeight:'700', cursor:'pointer', color:'#fff', background:'linear-gradient(135deg,#f59e0b,#d97706)' },
    reviewDone:{ marginTop:'10px', fontSize:'13px', color:'#d97706', fontWeight:'600', display:'flex', alignItems:'center', gap:'6px' },
    ...
    subBtnD:  { width:'100%', padding:'13px', background:'#ccc', color:'#fff', border:'none', borderRadius:'12px', fontSize:'15px', fontWeight:'700', cursor:'not-allowed', marginTop:'6px' },
  ...
  if (loading && mentors.length === 0) {
    return <div style={{ ...S.wrap, alignItems:'center', justifyContent:'center' }}><p style={{ color:'#1e4a76', fontSize:'16px', fontWeight:'600' }}>Memuat data...</p></div>;
  }
    ...
    <div style={S.wrap}>
      ...
      <main style={S.main}>
        ...
        {activeTab === 'search' && (<>
          ...
          <p style={{ fontSize:'13px', color:'#4a7a9b', marginBottom:'12px' }}>{filteredMentors.length} mentor ditemukan</p>
              ...
              <div key={m.id} style={{ ...S.card, cursor:'pointer' }} onClick={() => handleMentorClick(m)}>
                <div style={S.mRow}>
                  <div style={{ flex:1 }}>
                    ...
                    <div style={S.mPrice}>Rp {m.price_per_session?.toLocaleString('id-ID')}<span style={{ fontSize:'13px', fontWeight:'400', color:'#4a7a9b' }}> / sesi</span></div>
                  </div>
                  <button style={S.bookBtn} onClick={e=>{ e.stopPropagation(); handleMentorClick(m); }}>Pilih Mentor</button>
                </div>
              </div>
            ))
          }
        ...
        {activeTab === 'bookings' && (
          <div style={S.card}>
            ...
            {myBookings.length === 0
              ? <div style={S.empty}><IcCalendar /><p style={{ marginTop:'12px' }}>Belum ada booking.</p></div>
              : myBookings.map(b => (
                <div key={b.id} style={{ ...S.card, border:'1px solid rgba(42,107,158,0.15)', marginBottom:'12px' }}>
                  <div style={S.bRow}>
                    <div style={{ flex:1 }}>
                      ...
                      {/* Invoice di tab booking */}
                      {b.status === 'paid' && (
                        <div style={S.invPaid}><IcCheck /><span style={{ fontSize:'13px', color:'#155724', fontWeight:'700' }}>Lunas — Rp {Number(b.invoice_amount).toLocaleString('id-ID')}</span></div>
                      )}
                      {b.status === 'completed' && Number(b.invoice_amount) > 0 && (
                        <div>
                          <div style={S.invBox}><IcInvoice /><span style={{ fontSize:'13px', color:'#856404', fontWeight:'700' }}>Tagihan: Rp {Number(b.invoice_amount).toLocaleString('id-ID')}</span></div><br/>
                          <button style={payingId===b.id ? S.payBtnD : S.payBtn} disabled={payingId===b.id} onClick={()=>handlePayInvoice(b.id)}>
                            <IcPayment />{payingId===b.id ? 'Memproses...' : 'Bayar Sekarang'}
                          </button>
                        </div>
                      )}
                    </div>
        ...
        {activeTab === 'history' && (<>
          ...
          <div style={S.card}>
            <div style={S.secTitle}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#16a34a" strokeWidth="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
              Sesi Sedang Berlangsung
            </div>
            {ongoingSessions.length === 0
              ? <div style={S.empty}><IcClock /><p style={{ marginTop:'10px' }}>Tidak ada sesi berlangsung.</p></div>
                ...
                <div key={s.id} style={{ ...S.card, border:'1.5px solid #d4edda', marginBottom:'12px' }}>
                  <div style={S.bRow}>
                    <div style={{ flex:1 }}>
                      ...
                      <div style={S.mMeta}><IcCalendar />{new Date(s.date).toLocaleString('id-ID',{dateStyle:'long',timeStyle:'short'})}</div>
                      {s.meeting_link
                        ? <a href={s.meeting_link} target="_blank" rel="noopener noreferrer" style={S.meetLink}><IcLink /> Join Meeting</a>
                        : <div style={{ marginTop:'8px', fontSize:'13px', color:'#999', display:'flex', alignItems:'center', gap:'6px' }}><IcClock /> Menunggu mentor mengirim link meeting...</div>
                      }
                    </div>
          ...
          {/* Riwayat & Pembayaran */}
          <div style={S.card}>
            <div style={S.secTitle}><IcHistory /> Riwayat Sesi &amp; Pembayaran</div>
            {completedSessions.length === 0
              ? <div style={S.empty}><IcHistory /><p style={{ marginTop:'10px' }}>Belum ada riwayat sesi.</p></div>
              : completedSessions.map(s => {
                const isPaid      = s.status === 'paid';
                const isPending   = s.status === 'completed' || s.status === 'pending_payment';
                const alreadyReviewed = reviewStatus[s.id];
                return (
                  <div key={s.id} style={{ ...S.card, border:'1px solid rgba(42,107,158,0.15)', marginBottom:'12px' }}>
                    <div style={S.bRow}>
                      <div style={{ flex:1 }}>
                        <div style={S.mName}><IcMentor /> {s.mentor_name}</div>
                        <div style={S.mMeta}><IcCalendar />{new Date(s.date).toLocaleString('id-ID',{dateStyle:'long',timeStyle:'short'})}</div>
                        {s.notes && <div style={S.mMeta}><IcNote /> {s.notes}</div>}

                        {/* Lunas */}
                        {isPaid && (
                          <div style={{ marginTop:'12px', padding:'10px 14px', background:'#d4edda', borderRadius:'10px', border:'1px solid #c3e6cb', display:'inline-flex', alignItems:'center', gap:'8px' }}>
                            <IcCheck /><span style={{ fontSize:'13px', color:'#155724', fontWeight:'700' }}>✅ LUNAS — Rp {Number(s.invoice_amount).toLocaleString('id-ID')}</span>
                          </div>
                        )}
                        ...
                        {isPending && (
                          <div>
                            <div style={{ marginTop:'12px', padding:'10px 14px', background:'#fff3cd', borderRadius:'10px', border:'1px solid #ffc107', display:'inline-flex', alignItems:'center', gap:'8px' }}>
                              <IcInvoice /><span style={{ fontSize:'13px', color:'#856404', fontWeight:'700' }}>💰 Tagihan: Rp {Number(s.invoice_amount).toLocaleString('id-ID')}</span>
                            </div>
                            <button onClick={() => handlePayInvoice(s.id)} style={{ marginTop:'12px', display:'inline-flex', alignItems:'center', gap:'8px', border:'none', padding:'10px 22px', borderRadius:'10px', fontSize:'13px', fontWeight:'700', cursor:'pointer', color:'#fff', background:'#e53e3e' }}>
                              <IcPayment /> Bayar Sekarang
                            </button>
                          </div>
                        )}

                        {/* Tombol Review — hanya setelah paid */}
                        {isPaid && (
                          alreadyReviewed
                            ? <div style={S.reviewDone}><IcStarSm filled={true} /> Sudah direview</div>
                            : <button style={S.reviewBtn} onClick={() => {
                                setReviewModal({ open:true, bookingId:s.id, mentorName:s.mentor_name });
                                setReviewRating(0);
                                setReviewComment('');
                              }}>
                                <IcStarSm filled={false} /> Beri Rating &amp; Review
                              </button>
                        )}
                      </div>
                      <StatusBadge status={isPaid ? 'paid' : 'pending_payment'} />
                    </div>
                  </div>
                );
              })
          ...
          <div style={S.modal} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ background:'rgba(42,107,158,0.06)', borderRadius:'14px', padding:'16px', marginBottom:'18px' }}>
              <div style={{ fontSize:'24px', fontWeight:'800', color:'#2a6b9e', marginBottom:'8px' }}>
                Rp {selectedMentor.price_per_session?.toLocaleString('id-ID')}<span style={{ fontSize:'14px', fontWeight:'400', color:'#4a7a9b' }}> / sesi</span>
              </div>
            ...
            <p style={{ fontWeight:'700', color:'#1e4a76', marginBottom:'10px', display:'flex', alignItems:'center', gap:'6px' }}><IcCalendar /> Pilih Jadwal Tersedia</p>
            {selectedMentor.available_slots?.length > 0
              ? selectedMentor.available_slots.map(slot => (
                <div key={slot.id} style={{ ...S.slotItem, ...(selectedSlot?.id===slot.id ? S.slotOn : {}) }} onClick={() => setSelectedSlot(slot)}>
                  <input type="radio" readOnly checked={selectedSlot?.id===slot.id} />
                  <IcCalendar />
                  <span style={{ fontWeight:'500' }}>{slot.day || slot.date} — {slot.time}</span>
                </div>
              ))
              : <p style={{ color:'#aaa', fontSize:'13px', marginBottom:'16px' }}>Belum ada jadwal tersedia.</p>
            }
            <label style={{ display:'block', fontWeight:'700', color:'#1e4a76', marginTop:'14px', marginBottom:'6px', fontSize:'14px' }}>
              Catatan <span style={{ fontWeight:'400', color:'#aaa' }}>(opsional)</span>
            </label>
            <textarea placeholder="Topik yang ingin dipelajari..." value={bookingNotes} onChange={e=>setBookingNotes(e.target.value)} rows="3" style={S.textarea} />
            {bookingMsg && <div style={bookingMsg.includes('✅') ? S.msgOk : S.msgErr}>{bookingMsg}</div>}
            <button style={selectedSlot ? S.subBtn : S.subBtnD} onClick={handleBook} disabled={!selectedSlot}>
        ...
        <div style={S.overlay} onClick={() => setReviewModal({ open:false, bookingId:null, mentorName:'' })}>
          <div style={{ ...S.modal, maxWidth:'440px' }} onClick={e=>e.stopPropagation()}>
            <div style={S.mHead}>
              ...
              <button style={S.closeBtn} onClick={() => setReviewModal({ open:false, bookingId:null, mentorName:'' })}><IcClose /></button>
            </div>
            ...
            <div style={{ marginBottom:'20px' }}>
              ...
              {reviewRating > 0 && (
                <p style={{ fontSize:'13px', color:'#f59e0b', fontWeight:'600', marginTop:'8px' }}>
            ...
            <button onClick={handleSubmitReview} disabled={submittingReview || reviewRating===0}
              style={reviewRating>0 && !submittingReview ? S.subBtn : S.subBtnD}>
